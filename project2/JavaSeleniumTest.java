...
public class JavaSeleniumTest {
    ...
    public JavaSeleniumTest(){
        ...
        //Selenium WebDriver에서 일어나는 WebSocket애 대한 해결책.
    ...
    public static void main(String[] args) throws Exception {
        ...
        //File Folder5 = new File("D:/workspace/java/base/src/project/주식 차트/오늘의 증시");
        Folder1.mkdirs();
        Folder2.mkdirs();
        Folder3.mkdirs();
        Folder4.mkdirs();
        ...
        //FileUtils.cleanDirectory(new File("D:/workspace/java/base/src/project/주식 차트/오늘의 증시"));
        ...
        a:while(true){
            ...
            /* 
            //오늘의 증시
            List<String> resultList5= jt.getItemName5();
            jt.getItemInfo5(resultList5); */
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    /* 
    //오늘의 증시 - 코스피, 코스닥, 코스피 200
    public List<String> getItemName5(){
        List<String> itemNameList5 =new ArrayList<>();
        //try을 통한 예외 처리.
        try{
        driver.get(URL);
        //오늘의 증시
            int row =driver.findElements(By.xpath("//*[@id=\"content\"]/div[1]/div[2]/div[1]/div[2]")).size();
                                    
            for(int i=0; i<row ; i++){
            //Element(요소)의 속성값을 가져오는 .getAttribute를 사용하여 순위별 값을 가져옴
            //각 사이트 별 주소를 가져온다.
            //*[@id="content"]/div[1]/div[2]/div[1]/div[2]/div[1]/div[1]/a
            String data= driver.findElement(By.xpath("//*[@id=\"content\"]/div[1]/div[2]/div[1]/div[2]/div["+i+"]/div[1]/a")).getAttribute("href");
            //sise/sise_index.naver?code=(KOSPI, KOSDAK,KOSPI200) 으로 나오는데
            int index = data.lastIndexOf("=");
            String code=data.substring(index+1).trim(); //substring - 문자열 자르기 (=split) , .trim() 문자열 앞 뒤의 공백을 제거함.                        
            itemNameList5.add(code);
            }
        }catch(Exception e){
        e.printStackTrace();
        }
        return itemNameList5;
    }

                        
    public void getItemInfo5(List<String> resultList5){
        //불러온 웹 크롤링 정보 및 주소 출력하기.
        for(String code: resultList5){ 
        //웹 크롤링 주소 출력
        String param="?";
        param +="code="+ code;
                        
        String newUrl= "https://finance.naver.com/size/size_index.naver"+param;
        System.out.println(newUrl);
        ...
        String 이름 =driver.findElement(By.xpath("//*[@id=\"contentarea_left\"]/div[1]/h3")).getText();
        try{
            File scrFile = ((TakesScreenshot)driver.findElement(By.xpath("//*[@id=\"contentarea_left\"]/div[2]/div"))).getScreenshotAs(OutputType.FILE);
            FileUtils.copyFile(scrFile, new File("D:/workspace/java/base/src/project/주식 차트/오늘의 증시/"+이름+".png"));
            }catch(IOException e){
                e.printStackTrace();}
